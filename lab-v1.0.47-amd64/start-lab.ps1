...
# Create shared Docker network (FIXED: ตรวจสอบก่อนสร้าง ป้องกัน Error)
Write-Host "[1/4] Creating shared network 'lab-network'..." -ForegroundColor Yellow
$NetExists = & docker network ls --format "{{.Name}}" | Where-Object { $_ -eq "lab-network" }
if (-not $NetExists) {
    $null = & docker network create lab-network
...
# Connect waltid containers to lab-network (FIXED: ตรวจสอบก่อนเชื่อมต่อ ป้องกัน Endpoint Error)
...
foreach ($CID in $WaltidContainers) {
    if (-not [string]::IsNullOrWhiteSpace($CID)) {
        ...
        $NetworkCheck = & docker inspect -f '{{json .NetworkSettings.Networks}}' $CID
        if ($NetworkCheck -notmatch '"lab-network"') {
            # ปิด Error ชั่วคราวเพื่อซ่อนคำเตือนของ Docker
            $OldPref = $ErrorActionPreference
            $ErrorActionPreference = "SilentlyContinue"
            try { $null = & docker network connect lab-network $CID 2>&1 } catch {}
...
# 🟢 ปรับปรุง: ปิดโหมดดุร้ายชั่วคราว ป้องกันคำเตือนทำระบบพัง
$OldPref = $ErrorActionPreference
$ErrorActionPreference = "SilentlyContinue"

while ($true) {
    # 🟢 ปรับปรุง: ส่งรหัสผ่านผ่าน -e MYSQL_PWD เพื่อไม่ให้ MySQL พ่น Warning
    $null = & docker exec -e MYSQL_PWD="$DbPass" verifier-mysql mysqladmin ping -uroot --silent 2>&1
...
# 🟢 ปรับปรุง: ปรับคำสั่ง query ให้ส่งรหัสผ่านแบบปลอดภัยเช่นกัน
$SessionCount  = (& docker exec -e MYSQL_PWD="$DbPass" verifier-mysql mysql -uroot verifier --silent --skip-column-names -e "SELECT COUNT(*) FROM dbverifiersession;" 2>&1).Trim()
$ResponseCount = (& docker exec -e MYSQL_PWD="$DbPass" verifier-mysql mysql -uroot verifier --silent --skip-column-names -e "SELECT COUNT(*) FROM dbverifierresponse;" 2>&1).Trim()

# คืนค่าโหมดปกติให้ PowerShell
$ErrorActionPreference = $OldPref
